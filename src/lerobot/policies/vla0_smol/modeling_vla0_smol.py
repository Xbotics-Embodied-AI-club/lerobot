...
class VLA0(nn.Module):
    def __init__(self, config: VLA0SmolConfig):
        ...
        # 快速图像处理器显式构造并传入（取代上游对 SmolVLMProcessor 的运行时改写）。
        # 上游另有一处把 SmolVLMModel.inputs_merger 包一层 dtype 转换以支持混合精度训练；
        # 在 float32 下 dtype 已一致、该转换为恒等，故此处不引入对 transformers 的运行时改写。
